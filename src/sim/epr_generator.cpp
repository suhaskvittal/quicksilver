...
EPR_GENERATOR::EPR_GENERATOR(double freq_khz, MEMORY_MODULE* owner, size_t buffer_cap)
    ...
    max_decoupled_loads_(buffer_cap/4)
{}
...
{
    if (epr_buffer_occu_ == 0)
        throw std::runtime_error("EPR_GENERATOR::alloc_decoupled_load: no EPR pairs available");
    if (inflight_decoupled_loads_ == 0)
        throw std::runtime_error("EPR_GENERATOR::alloc_decoupled_load: this decoupled load was not declaredk");
    if (decoupled_loads_.size() >= max_decoupled_loads_)
        throw std::runtime_error("EPR_GENERATOR::alloc_decoupled_load: too many decoupled loads");

    epr_buffer_occu_--;
    decoupled_loads_.push_back(q);
    inflight_decoupled_loads_--;
}

void
EPR_GENERATOR::exchange_decoupled_load(QUBIT old_q, QUBIT new_q)
{
    auto q_it = std::find(decoupled_loads_.begin(), decoupled_loads_.end(), old_q);
    if (q_it == decoupled_loads_.end())
        throw std::runtime_error("EPR_GENERATOR::exchange_decoupled_load: attempting to exchange non-existent decoupled load");

    *q_it = new_q;
}

void
EPR_GENERATOR::remove_decoupled_load(QUBIT q)
{
    auto q_it = std::find(decoupled_loads_.begin(), decoupled_loads_.end(), q);
    if (q_it == decoupled_loads_.end())
        throw std::runtime_error("EPR_GENERATOR::remove_decoupled_load: attempting to remove non-existent decoupled load");
    decoupled_loads_.erase(q_it);
}
...
void
EPR_GENERATOR::mark_inflight_decoupled_load()
{
    inflight_decoupled_loads_++;
}
...
{
    return (decoupled_loads_.size() + inflight_decoupled_loads_ < max_decoupled_loads_) && has_capacity();
}
...
{
    std::cerr << "\tbuffer occu: " << epr_buffer_occu_ << "\n";
    std::cerr << "\tdecoupled loads: ";
    for (auto q : decoupled_loads_)
        std::cerr << " " << q;
    std::cerr << "\n";
}
