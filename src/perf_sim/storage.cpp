...
Storage::do_load(Qubit* q)
{
    ...
    auto result = do_memory_access(load_latency, Access::LOAD);
...
Storage::do_store(Qubit* q)
{
    ...
    auto result = do_memory_access(store_latency, Access::STORE);
...
Storage::do_coupled_load_store(Qubit* ld, Qubit* st)
{
    ...
    auto result = do_memory_access(load_latency + store_latency, Access::COUPLED_LOAD_STORE);
...
Storage::access_result_type
Storage::do_memory_access(cycle_type access_latency, Access type)
{
    ...
    case Access::STORE:
        ...
        break;
    case Access::COUPLED_LOAD_STORE:
...
cycle_type
Storage::adapter_access(std::vector<cycle_type>::iterator adapter_it, Access type)
{
    if (type == Access::STORE || load_latency == 0)
