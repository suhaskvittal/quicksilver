...
for (j, which) in enumerate(['baseline', 'compute', 'memory', 'factory_all_levels', 'factory_only_l1']):
    ...
    for (i,w) in enumerate(base_workloads):
        if which == 'baseline':
            ...
            cmd = f'./build/qs_sim_mem benchmarks/bin/{w} {sim_inst_count} {total_inst} -dsma -jit --cmp-sc-count {CMP_COUNT} --fact-phys-qubits-per-program-qubit {FACT_PHYS_QUBITS_PER_PROGRAM_QUBIT} --mem-bb-num-modules {MEM_BB_NUM_MODULES} &> {output_path}'
            print(cmd)
        else:
            for rf in [0.05, 0.1, 0.2]:
                output_path = f'{output_folder}/{compile_to[i]}_{int(rf*100)}.out'
                total_inst = total_inst_count[i]
                cmd = f'./build/qs_sim_mem benchmarks/bin/{w} {sim_inst_count} {total_inst} -dsma -jit --cmp-sc-count {CMP_COUNT} --fact-phys-qubits-per-program-qubit {FACT_PHYS_QUBITS_PER_PROGRAM_QUBIT} --mem-bb-num-modules {MEM_BB_NUM_MODULES} --qh-reduce-which {j} --qh-reduce-fraction {rf} &> {output_path}'
                print(cmd)
